...
scp=$sdir/wav.scp
hcopybin=/home/anirudhs/tmp/anu/rohit/generate_feats_anu_mc_gn.sh
nj=40
...
echo "pwdr is ${pwdr}"
python3 utils/split_scp.py --splits $nj --file ${pwdr}/filelist.scp --prefix filelist --dest ${pwdr}/splits
...
for job in `seq 1 $nj`; do
 ...
 qsub -l hostname=!compute-0-5 -e $err -o $out -S /bin/bash $hcopybin ${pwdr}/newfea/split_$job \
